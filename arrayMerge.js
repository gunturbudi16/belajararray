...
- Wajib menuliskan pseudocode atau algoritma.
  Tidak 
  ada pseudocode / pseudocode tidak match dengan kode maka indikasi
...
function arrayMerge(firstData, secondData) {
    ...
    for (var i = 0; i < secondData.length; i++) {
        arr.push(secondData[i]) //gabungin data
    }
    var temporary = [] //buang data yg sama melalui array penampung
    for (var i = 0; i < arr.length; i++) {
        var flek = true
        for (var j = 0; j < temporary.length; j++) {
            if (arr[i] == temporary[j]) {
                flek = false
            }
        }
        if (flek) {
